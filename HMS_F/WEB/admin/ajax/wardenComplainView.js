...
function getAllWardenComplaintView() {
  $.ajax({
      method: "GET",
      url: "http://localhost:8080/api/complaints/wardenView",
      success: function(data, status) {
          console.log("Status:", status); // Print the status
          console.log("Data:", data); // Print the data
          // Clear existing table rows
          $('#wardenComplainTable tbody').empty();
          ...
          for (let i = 0; i < data.length; i++) {
              ...
              if (complaint.status === 'pending') { // Check if the status is 'pending'
                  ...
                  $('#wardenComplainTable tbody').append(newRow);
              }
          }
      },
